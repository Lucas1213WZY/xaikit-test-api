...
DATASETS = {
    "forest_cover": {"name": "Forest Cover", "appId": "forest_cover", "model": "xgboost", "expMethod": "shap", "blocks": forest_blocks},
    "wine_quality": {"name": "Wine Quality", "appId": "wine_quality", "model": "mlp", "expMethod": "lime", "blocks": wine_blocks},
    "adult": {"name": "Adult Income", "appId": "adult", "model": "xgboost", "expMethod": "lime", "blocks": adult_blocks},
}
...
def make_ai_loader(ds_cfg, xai_type):
    ...
    return (
        ...
        .filter_loader(
            lambda df, app=ds_cfg["appId"], mdl=ds_cfg["model"], exp=ds_cfg["expMethod"]: make_mask(
                df, appId=app, modelName=mdl, expMethod=exp
            )
        )
    )
...
def class_labels(ds_cfg):
    df_metadata = pd.read_csv(DATA_DIR / "metadata.csv")
    row = df_metadata[df_metadata["appId"] == ds_cfg["appId"]]
    if row.empty:
        return {0: "0", 1: "1"}
    row = row.iloc[0]
...
def target_instance_id(ds_cfg, tested_condition, instance_number, block_index):
    ...
    if instance_number < 0 or instance_number >= len(ids):
        raise IndexError(f"instance_number must be 0..{len(ids) - 1} for {ds_cfg['appId']} {key}.")
...
def target_instance_location(ds_cfg, tested_condition, instance_id, block_index=None):
    ...
    label = ds_cfg.get("appId", ds_cfg.get("name", "dataset"))
